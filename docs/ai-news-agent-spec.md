...
- 重要度スコアリングと降順ソート
- Markdown ファイルへの出力
- MCP サーバーとして Claude Code から呼び出し可能にする
- **プロファイルによる検索条件の固定化**（名前付き設定ファイルで毎回の入力を省略）
- **記事 DB への永続化**（過去記事の蓄積・embedding 保存）
- **トレンド検出**（過去 N 日との比較による「新規 / 継続中 / 急浮上」分類）
- **フィードバックループ**（既読・有用記事の記録によるスコアリング精度の自己改善）
...
名前付きプロファイルが存在する場合、TUI 対話をスキップして即実行できる。

```bash
fetch_ai_news morning        # 毎朝の定常チェック
fetch_ai_news weekly         # 週次レビュー
fetch_ai_news langraph-watch # 特定ライブラリの更新追跡
```

プロファイルは `profiles.yaml` で管理する。TUI 対話は「新しいプロファイルを作るとき」と「アドホックな検索をするとき」のみ起動する。

```yaml
# profiles.yaml の例
morning:
  abstractionLayer: middle
  query: "LLM agent MCP tool"
  limit: 10
  sources: [hacker_news, zenn, dev.to]

weekly:
  abstractionLayer: macro
  query: "AI industry trend regulation"
  limit: 20
  sources: [techcrunch, wired, hacker_news]

langraph-watch:
  abstractionLayer: micro
  query: "LangGraph"
  limit: 5
  sources: [github_releases, official_docs]
```
...
過去記事を蓄積し、トレンド検出とフィードバックループの基盤を提供する。

```
article_db/
  articles.db       # 記事メタデータ（SQLite）
  embeddings/       # embedding ベクトル（LanceDB / Chroma）
  feedback.jsonl    # フィードバックログ（行指向 JSON）
```
...
今日の記事（embedding）
    ↓
過去 N 日分の記事 DB と類似度比較
    ↓
  類似度 < 閾値     → 「🆕 新規」
  類似度 ≥ 閾値 × 1日 → 「📈 急浮上」（初出が昨日以降）
  類似度 ≥ 閾値 × 3日 → 「🔄 継続中」（3日以上続いているトレンド）
```

出力 md のヘッダーに「今週のトレンドサマリー」セクションを追加する。

### プロファイルシステム

```
profiles.yaml       # ユーザー定義プロファイル
```

MCP ツールのインターフェースを拡張し、プロファイル名での呼び出しを受け付ける：

```typescript
server.tool("fetch_ai_news", {
  profile: z.string().optional(),  // profiles.yaml のキー名
  // profile 未指定時は以下のフィールドを使用
  query: z.string().optional(),
  abstractionLayer: z.enum(["macro", "middle", "micro"]).optional(),
  targetSources: z.array(z.string()).optional(),
...
生成した md の各記事にチェックボックスを付与する。

```markdown
- [x] LangGraph v0.3.0 リリース — ストリーミング対応が強化  ← ユーザーがチェック
- [ ] OpenAI が新モデルを発表
```

次回実行時に前回の md を読み取り、チェック済み記事のタイトル・ソース・タグをフィードバックとして `scoreArticle` のプロンプトに注入する。これにより、ユーザーが「有用」と判断した記事の特徴が次回スコアリングに反映される。
...
- [ ] **`profiles.yaml` の実装**（名前付きプロファイルで即実行できる状態にする）

**判断基準**: 自分が実際に毎朝使えるレベルになっているか

---

### Phase 2 — 内側エージェント化 + 永続化（目安: Phase 1 完了から 3〜4 週間）

**目標**: 固定パイプラインをエージェントループに置き換え、記事 DB を導入する
...
- [ ] **記事 DB の永続化**（SQLite + LanceDB / Chroma で過去記事と embedding を蓄積）
- [ ] **フィードバックループの基礎実装**（md のチェックボックスを次回実行時に読み取り、スコアリングプロンプトに反映）

**学習価値**: Multi-Query・LLM-as-judge・embedding 重複除去・ベクトル DB 永続化は仕事の RAG 改善に直結する

---

### Phase 3 — MCP サーバー化・二重エージェント構造 + トレンド検出（目安: Phase 2 完了から 2〜3 週間）

**目標**: Claude Code の TUI 上で対話から md 生成まで完結させる。過去 DB を使ったトレンド検出を加える

- [ ] ai-news-agent を MCP サーバーとして再実装（`@modelcontextprotocol/sdk` 使用）
- [ ] `fetch_ai_news` ツールの公開（プロファイル名 or NewsRequest インターフェース）
- [ ] **TUI 対話フェーズの実装（Claude Code 側）**
- [ ] **abstractionLayer の判定ロジック実装**
- [ ] `claude_desktop_config.json` への登録・Claude Code との疎通確認
- [ ] **トレンド検出の実装**（過去 N 日との embedding 比較で「新規 / 継続中 / 急浮上」を分類）
- [ ] md ヘッダーに「今週のトレンドサマリー」セクションを追加
...
| Embedding（重複除去・トレンド検出） | Voyage AI / OpenAI embedding | Phase 2 で導入 |
| 記事 DB（メタデータ） | SQLite（`better-sqlite3`） | Phase 2 で導入 |
| ベクトル DB（永続化） | LanceDB または Chroma | Phase 2 で導入。ローカル動作・軽量を優先 |
| プロファイル設定 | `profiles.yaml`（`js-yaml` でパース） | Phase 1 から使用 |
...
- **Phase 1**: プロファイル名 1 つでその日のダイジェスト md が生成できる
- **Phase 2**: 重複記事が目視で明らかに減り、重要度順が体感で正しい。量・質が不足した日もフォールバックで補完される。使い続けるほどスコアリング精度が上がる実感がある
- **Phase 3**: TUI 上で対話 → ニュース収集 → md 生成が完結する。md に「新規 / 継続中 / 急浮上」の分類が付き、今週のトレンドサマリーが読める
...
- **ベクトル DB 永続化 + トレンド検出**（時系列の記事 DB を使った「新規 / 継続 / 急浮上」分類）
- **フィードバックループによるスコアリング自己改善**（使い続けるほど精度が上がる仕組み）
